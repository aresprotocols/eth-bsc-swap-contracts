...
contract BSCSwapAgentImpl is Context, Initializable {
    ...
    event SwapPairCreated(
        bytes32 indexed ethRegisterTxHash,
    ...
    //     require(
    //         swapMappingETH2BSC[erc20Addr] == address(0x0),
    ...
    function createSwapPair(
        ...
        require(
            swapMappingBSC2ETH[bep20Addr] == address(0x0),
            "duplicated swap pair"
        );

        string memory name = IERC20Query(bep20Addr).name();
        string memory symbol = IERC20Query(bep20Addr).symbol();
        uint8 decimals = IERC20Query(bep20Addr).decimals();
     ...
     * @dev swapBSC2ETH
     */
    function swapBSC2ETH(address bep20Addr, uint256 amount)
        external
        payable
        notContract
        returns (bool)
